<script type="text/javascript">
     ...
     document.addEventListener('DOMContentLoaded', () => {
          ...
          <?php foreach ($getDetalleVenta->result() as $row) : ?>
               carritoProductos = [...carritoProductos, {
                    ...
                    precio: <?php echo $row->precioTotal ?>,
                    precioVenta: <?php echo $row->precioTotal ?>,
                    stock: <?php echo $row->stock_cantidad_total ?>,
                    cantidad: <?php echo $row->cantidad ?>,
                    total: <?php echo ($row->cantidad * $row->precioTotal) ?>,
               }]
     ...
     const devolverAumentarInput = document.querySelector('#venta-faltante-sobrante-input');

     devolverAumentarInput.addEventListener("input", completarPago);

     function completarPago(e) {
          const tVentas = Number(document.querySelector('#tVentas').value);
          e.target.value;
          const cantidaddinerorecibido = Number(document.querySelector('.cantidad-dinero-recibido').value);
          
          const tRecibidospan = document.querySelector('.tRecibido-span');

          //Cliente debe aumentar el monto
          
          if (tVentas > cantidaddinerorecibido) {
               const total = cantidaddinerorecibido + Number(e.target.value);
               tRecibidospan.textContent = total;
          }

          //Debemos devolcer dinero al cliente
          if (tVentas < cantidaddinerorecibido) {
               const total = cantidaddinerorecibido - Number(e.target.value);
               tRecibidospan.textContent = total;
     ...
     function crearHTML() {
          ...
          if (carritoProductos.length > 0) {
               carritoProductos.forEach((prod, i) => {
                    ...
                    row.innerHTML = `
          <input type="hidden" id="idProducto" value="${idProducto}" name="idProductoNuevo[]">
               <td>${++i}</td>
               <td>${codigo}</td>
               <td>${nombreProducto}</td>
               <td class="stock">${stock}</td>
               <td><input type="number" class="form-control cantidad" placeholder="Cantidad" id="cantidad" name="cantidadNuevo[]" value="${cantidad}" max="${stock}"></td>
               <td class="precioVenta">${precioVenta}</td>
               
               <input type="hidden" name="precioVentaNuevo[]" value="${precioVenta}">
     ...
     function actualizarFaltanteSobrante() {


          const cantidaddinerorecibido = Number(document.querySelector('.cantidad-dinero-recibido').value);
          const etiquetaFaltanteSobrante = document.querySelector('.venta-faltante-sobrante');
          const inputFaltanteSobrante = document.querySelector('#venta-faltante-sobrante-input');
          const tVentas = Number(document.querySelector('#tVentas').value);


          //Cliente debe aumentar el monto
          etiquetaFaltanteSobrante.style.color = "red";
          etiquetaFaltanteSobrante.style.display = "block";
          if (tVentas > cantidaddinerorecibido) {
               const diferencia = tVentas - cantidaddinerorecibido;
               etiquetaFaltanteSobrante.textContent = `Faltante: ${diferencia}`;
               inputFaltanteSobrante.style.display = "block";
          }

          //Debemos devolcer dinero al cliente
          if (tVentas < cantidaddinerorecibido) {
               inputFaltanteSobrante.style.display = "block";
               const diferencia = cantidaddinerorecibido - tVentas;
               etiquetaFaltanteSobrante.textContent = `Devolver: ${diferencia}`;

          }

          if (tVentas == cantidaddinerorecibido) {
               inputFaltanteSobrante.style.display = "none";

               etiquetaFaltanteSobrante.style.display = "none";
     ...
     $(document).ready(function() {
          ...
          $('#txtCliente').val('<?php echo $getVenta->row()->idCliente ?>').change();
